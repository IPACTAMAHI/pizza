  ...
  while (true) {
    var entered = await showModal({
      ...
      message: (warning ? warning + '\n\n' : '') +
        'Заполните оба поля настоящими данными — это важно.\n\n' +
        'Имя и фамилия — по ним управляющий понимает, кто именно просит доступ. ' +
        'Из списка «Олегов» без фамилии выбрать невозможно, и заявку просто отклонят.\n\n' +
        'Телефон — единственный способ связаться с вами по рабочим вопросам: ' +
        'подтвердить заявку, позвонить по смене, уточнить что-то по рецепту. ' +
        'Управляющий может позвонить перед тем, как открыть доступ.\n\n' +
        'Эти данные видят только управляющий вашего заведения и администраторы книги. ' +
        'Вымышленные имя или номер означают отказ в доступе.',
      ...
      withFields: (function() {
        var fields = [
          { key: 'name', label: 'Имя и фамилия', value: typed, placeholder: 'Олег Петров',
            required: true, minLength: 2, hint: 'Как в жизни — по нему вас узнает управляющий' },
          { key: 'phone', label: 'Телефон', value: typedPhone, placeholder: '+380 67 123 45 67',
            inputType: 'tel', required: true, minDigits: 9, hint: 'Для связи по работе. На него могут позвонить' }
        ];

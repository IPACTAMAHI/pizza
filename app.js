...
/* Переход по ссылке через реальный <a>-тег вместо window.location.href.
   Разница принципиальна для мобильных браузеров: iOS Safari и Chrome
   на Android показывают системное окно "Сайт пытается открыть другое
   приложение — Разрешить/Запретить" КАЖДЫЙ РАЗ, когда переход на
   ссылку типа t.me/wa.me инициирован СКРИПТОМ (window.location.href = ...),
   даже если до этого пользователь уже сто раз разрешал открытие —
   это не запоминается и не настраивается, потому что браузер не отличает
   такой переход от вредоносного автоматического редиректа. У самого
   диалога нет опции "больше не спрашивать".
   А вот переход по НАСТОЯЩЕЙ ссылке <a href="..."> (даже если клик по
   ней эмулирован через .click(), но синхронно, в рамках того же
   обработчика нажатия кнопки) браузер распознаёт как обычный переход
   по гиперссылке — ровно так же, как открытие Universal Link/ссылки
   мессенджера по тапу пальцем на письме или в другом приложении. Для
   таких переходов ни iOS, ни Android никакого подтверждающего окна не
   показывают вообще — приложение открывается сразу. */
function navigateViaRealLink(url) {
  var a = document.createElement('a');
  a.href = url;
  a.rel = 'noopener';
...
/* Копирует отчёт текущего шага в буфер обмена и открывает приложение
   ...
   На телефоне переходим по ссылке через настоящий <a>-тег (см.
   navigateViaRealLink выше) — это не только надёжнее передаёт
   управление установленному приложению, но и не вызывает у ОС
   повторного окна "Разрешить открытие приложения?" при каждом нажатии.
...
function sendAllPurchaseOpenAndCopy() {
  ...
  if (isMobileDevice()) {
    navigateViaRealLink(resolved.url);
  } else {
